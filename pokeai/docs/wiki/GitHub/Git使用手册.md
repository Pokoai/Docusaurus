...
git diff HEAD   // 查看本次提交与上次提交的不同，建议在 commit 前先执行该命令

git add readme.md	// 将文件 readme.md 放到暂存区
git add .   // 针对所有变动文件

git commit -m "fix bug"	(修改内容说明)  // 提交修改文件至本地仓库，并做修改说明

git push origin main	// 推送到远程仓库（origin）的 main 分支
git push -f origin main     // 强制推送，用本地全覆盖远程仓库
git push -u origin main     // 初次推送使用

git log     // 查看提交日志
git log --online    // 精简查看日志，推荐使用

git clone 远程仓库 ssh 链接     // 从远程仓库下载到本地
